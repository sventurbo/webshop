...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'addToShoppingCart') {
        ...
        addToShoppingCart($productID, $con); //Wenn Button gedrückt, führe das Insert Statement aus
    }
}
...
function addToShoppingCart($productID, $con) {
    if (isset($_SESSION['email'])) {
        ...
        $stmt = $con->prepare("SELECT id FROM users WHERE email=:email"); //Prüfe ob Nutzer vorhanden
    ...
    $stmt_check = $con->prepare("SELECT shoppingID,productID,Amount,userID FROM shoppingcart WHERE productID = :productID AND userID =:userID"); //Prüfe ob Produkt schon im Warenkorb, falls ja, dann nur die Anzahl erhöhen
    ...
    if ($existing_product) {
        ...
        $stmt = $con->prepare("INSERT INTO shoppingcart (productID, Amount, userID) VALUES (:productID, 1, :userID)"); //Wenn PRodukt noch nicht im Warenkorb, füge es hinzu
...
function deleteshoppingcart($con){
    ...
    if (isset($_SESSION['email'])) {
        ...
        if($userID) {
            ...
            $stmt_delete = $con->prepare("DELETE FROM shoppingcart WHERE userID =:userID"); //Lösche des alle Produkte des Nutzers
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { //Php Listener, der auf die Ajax-Nachricht reagiert
...
<html lang="de" data-bs-theme="dark" data-lt-installed="true">
    ...
    <body>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <button id="<?= $productsDB['id'] ?>" class="btn btn-secondary" onclick="addToCart(); triggerPHPFunction('<?= $productsDB['id'] ?>');">Kaufen</button>
                            </div>
        ...
        window.addEventListener('beforeunload', function(event) {
            deleteshoppingcart($con);
        }); // Wenn du die Seite verlässt, neu lädtst oder ganz schließt, dann führe den Code aus, dieser löscht alle Produkte aus dem Warenkorb
    ...
    function deleteshoppingcart($con) {
        ...
        $.ajax({
            url: 'store.php',
            type: 'POST',
            data: { action: 'deleteshoppingcart'},
